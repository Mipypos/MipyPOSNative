...
class ExportService {
  ...
  static Future<void> _write(String name, List<List<dynamic>> rows) async { final d = await _dir(); await d.create(recursive: true); await File('${d.path}/$name.csv').writeAsString(rows.map((r) => r.map(_csv).join(',')).join('\n')); final x = Excel.createExcel(); final s = x['Reporte']; for (final r in rows) { s.appendRow(r); } await File('${d.path}/$name.xlsx').writeAsBytes(x.encode()!); }
  static String _csv(dynamic v) { final s = '$v'; return s.contains(',') ? '"${s.replaceAll('"', '""')}"' : s; }
  ...
  static Future<void> exportProductosVendidos() async { final rows = <List<dynamic>>[['Producto', 'Cantidad', 'Total']]; final map = <String, List<num>>{}; for (final i in await DBService.getSaleItems()) { final n = '${i['name'] ?? 'Producto'}'; final q = i['quantity'] is num ? i['quantity'] as num : 0; final total = i['subtotal'] is num ? i['subtotal'] as num : q * (i['price'] is num ? i['price'] as num : 0); map[n] = [(map[n]?.first ?? 0) + q, (map[n]?.last ?? 0) + total]; } for (final e in map.entries) { rows.add([e.key, e.value[0], e.value[1]]); } await _write('productos_vendidos', rows); }
